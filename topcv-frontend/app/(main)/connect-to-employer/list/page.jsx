...
import {
    Building2, CheckCircle, XCircle, MessageSquare,
    ChevronLeft, ChevronRight, MapPin, Bookmark, X, Clock, Sparkles,
} from 'lucide-react';
...
function StatusBadge({ status }) {
    ...
    return (
        <span style={{ padding: '3px 10px', borderRadius: '20px', background: s.bg, color: s.color, fontSize: '11px', fontWeight: '600' }}>
            {s.label}
        </span>
    );
}
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{
                    width: '48px', height: '48px', borderRadius: '10px', border: '1px solid #e5e7eb',
...
function SidebarJobCard({ job, saved, onToggleSave, onDismiss }) {
    ...
    const salary = (() => {
        ...
        const fmt = (n) => n >= 1000000 ? `${(n / 1000000).toFixed(0)} - ${(n / 1000000).toFixed(0)} triệu` : `${n.toLocaleString()}đ`;
        if (job.salaryMin && job.salaryMax) {
            const fmtMin = job.salaryMin >= 1000000 ? `${(job.salaryMin / 1000000).toFixed(0)}` : `${job.salaryMin}`;
            const fmtMax = job.salaryMax >= 1000000 ? `${(job.salaryMax / 1000000).toFixed(0)} triệu` : `${job.salaryMax}đ`;
        ...
        >
            <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', paddingRight: '44px' }}>
                <div style={{
                    ...
                    {logo
                        ? <Image src={logo} alt={companyName} width={40} height={40} unoptimized style={{ objectFit: 'contain', width: '100%', height: '100%' }} />
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#6b7280', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: '1px' }}>
                        {companyName}
                    </div>
                    <div style={{ display: 'flex', gap: '6px', marginTop: '4px', flexWrap: 'wrap', alignItems: 'center' }}>
                        <span style={{ fontSize: '11px', color: GREEN, fontWeight: '600', background: '#f0fdf4', borderRadius: '4px', padding: '1px 6px' }}>
                            {salary}
                        </span>
                        {location && (
            ...
            <div style={{ position: 'absolute', top: '8px', right: '8px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                <button
                    onClick={e => { e.stopPropagation(); onDismiss(); }}
                    title="Ẩn gợi ý"
                    style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', color: '#d1d5db', lineHeight: 0 }}
                >
                    <X size={13} />
                </button>
                <button
                    ...
                    style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', color: saved ? GREEN : '#d1d5db', lineHeight: 0 }}
                >
...
function JobSuggestionSidebar({ isAuthenticated }) {
    ...
    return (
        <div style={{
            ...
            {loading ? (
                <div style={{ padding: '24px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>Đang tải...</div>
            ) : jobs.length === 0 ? (
                <div style={{ padding: '24px', textAlign: 'center' }}>
                    <div style={{ fontSize: '13px', color: '#6b7280', marginBottom: '10px' }}>Chưa có gợi ý phù hợp</div>
                    <Link href="/cai-dat-goi-y-viec-lam" style={{ fontSize: '12px', color: GREEN, fontWeight: '600', textDecoration: 'underline' }}>
                        Cài đặt gợi ý
                    </Link>
                </div>
            ) : (
                    ...
                    <div style={{ padding: '12px 16px', borderTop: '1px solid #f3f4f6' }}>
                        <Link
                            ...
                            style={{
                                ...
                                fontSize: '13px', color: GREEN, fontWeight: '600', textDecoration: 'none',
                            }}
                        >
                            Xem thêm →
                        </Link>
...
export default function ConnectListPage() {
    ...
    useEffect(() => {
        if (isAuthenticated) { setPage(1); fetchRequests(activeTab, 1); }
    }, [activeTab, isAuthenticated, fetchRequests]);

    useEffect(() => {
        if (isAuthenticated && page > 1) { fetchRequests(activeTab, page); }
