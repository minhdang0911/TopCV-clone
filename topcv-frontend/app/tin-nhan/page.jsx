...
export default function CandidateChatPage() {
    ...
    useEffect(() => {
        ...
        connect();
        return () => { wsDeadRef.current = true; if (timer) clearTimeout(timer); wsRef.current?.close(); };
        ...
        return (
            <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '15px', color: '#374151', marginBottom: '12px' }}>Vui lòng đăng nhập để xem tin nhắn</div>
                    <button onClick={() => router.push('/login')} style={{ background: GREEN, color: 'white', border: 'none', borderRadius: '8px', padding: '10px 24px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>Đăng nhập</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div style={{ display: 'flex', height: 'calc(100vh - 60px)', background: '#f3f4f6', overflow: 'hidden' }}>
            ...
            {(!isMobile || showSidebar) && (
                <div style={{ width: isMobile ? '100%' : '320px', flexShrink: 0, background: 'white', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: '16px', borderBottom: '1px solid #f3f4f6' }}>
                        <div style={{ fontSize: '16px', fontWeight: '700', color: '#111827', marginBottom: '10px' }}>Tin nhắn</div>
                        <div style={{ position: 'relative' }}>
                            ...
                            <input
                                ...
                                style={{ width: '100%', padding: '8px 10px 8px 30px', border: '1px solid #e5e7eb', borderRadius: '8px', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }}
                            />
                        </div>
                    </div>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto' }}>
                        {loadingConvs ? (
                            ...
                            <div style={{ padding: '32px 16px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>Chưa có cuộc trò chuyện nào</div>
                        ) : (
                                    ...
                                    >
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2px' }}>
                                                <span style={{ fontSize: '13px', fontWeight: conv.unreadCount > 0 ? '700' : '600', color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ep.companyName}</span>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                <span style={{ fontSize: '12px', color: '#6b7280', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
                                                    {lastMsg ? (lastMsg.senderId === user?.id ? 'Bạn: ' : '') + lastMsg.content : 'Bắt đầu trò chuyện...'}
                                                </span>
                                                {conv.unreadCount > 0 && (
            ...
            {(!isMobile || !showSidebar) && (
                activeConvId && activeConv ? (
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#9ca3af' }}>
                        ...
                        <div style={{ fontSize: '13px', marginTop: '6px' }}>Hoặc bắt đầu nhắn tin từ trang chi tiết công việc</div>
                    </div>
                )
            )}
                ...
                <div style={{ width: '260px', flexShrink: 0, background: 'white', borderLeft: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', padding: '4px 0' }}>
                        {(() => {
                            ...
                            const jobs = epId
                                ? appliedJobs.filter(a => a.job?.employer?.id === epId)
                                : appliedJobs;
                            if (jobs.length === 0) {
                                return <div style={{ padding: '24px 16px', textAlign: 'center', color: '#9ca3af', fontSize: '12px' }}>Chưa có dữ liệu</div>;
                            }
                            return jobs.map(app => {
                                ...
                                return (
                                    <div key={app.id} style={{ padding: '10px 12px', borderBottom: '1px solid #f3f4f6', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                                        <div style={{ width: '36px', height: '36px', borderRadius: '6px', border: '1px solid #e5e7eb', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9fafb' }}>
                                            {ep.logoUrl
                                                ? <img src={ep.logoUrl} alt={ep.companyName} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                                                : <span style={{ fontSize: '13px', fontWeight: '700', color: GREEN }}>{(ep.companyName || '?')[0]}</span>
                                            }
                                        </div>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ep.companyName}</div>
                                            <div style={{ fontSize: '12px', fontWeight: '600', color: '#111827', lineHeight: '1.3', marginBottom: '6px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{app.job?.title}</div>
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '4px' }}>
                                                <button onClick={handleChatRight} style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600', background: '#f0fdf4', color: GREEN, border: `1px solid ${GREEN}`, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                                    Nhắn tin
                                                </button>
                                                <a href={`/viec-lam/${app.job?.slug || app.job?.id}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: '11px', color: '#6b7280', textDecoration: 'none' }}>
                                                    Xem tin
                                                </a>
                                            </div>
                                        </div>
